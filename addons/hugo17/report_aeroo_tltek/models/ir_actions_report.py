...
class IrActionsReport(models.Model):
    ...
    open_browser = fields.Boolean(string='Open Browser', default=False)

    def report_action(self, docids, data=None, config=True):
        report_action = super().report_action(docids, data=data, config=config)
        if self.open_browser and report_action["report_type"] == 'qweb-pdf':
            report_action["report_type"] = "qweb-pdf-browser"
        elif self.report_type == 'aeroo':
            report_action["id"] = self.id
            report_action["out_format"] = self.aeroo_out_format_id.code
    ...
    def _get_aeroo_timezone(self, record):
        ...
        """
        return (
            safe_eval.safe_eval(self.aeroo_tz_eval, self._get_aeroo_variable_eval_context(record))
    ...
    def _render_aeroo(self, doc_ids, data=None, force_output_format=None):
        ...
        objects =  self.env[self.model].browse(doc_ids)
